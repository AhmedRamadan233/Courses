                    ...
                    <form action="{{ route('common_questions.update', ['common_questions' => $editedCommonQestions->id]) }}" method="post" enctype="multipart/form-data">
                            ...
                            <select class="form-control" id="parent_id" name="parent_id">
                                ...
                                @foreach ($categories as $category)
                                    @if ($category->parent_id == null)
                                        <option value="{{ $category->id }}" {{ $category->id == $editedCommonQestions->category->parent_id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endif
                                @endforeach
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="category_id" name="category_id">
                                @foreach ($categories as $category)
                                    @if ($category->parent_id !== null)
                                        <option value="{{ $category->id }}" {{ $category->id == old('category_id', $editedCommonQestions->category_id) ? 'selected' : '' }}>
                                            {{ $category->slug }}
                                        </option>
                                    @endif
                                @endforeach
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data.categories, function(index, category) {
                        $('#category_id').append('<option value="' + category.id + '" ' + (category.id == {{ $editedCommonQestions->category_id }} ? 'selected' : '') + '>' + category.slug + '</option>');
